...
func _physics_process(delta):
	...
	for i in range(rantai_permanen.size()):
		...
		jumlah_aktif += 1
		rantai_sehat.append(gerbong) # Masukkan ke daftar bersih
...
func tambah_gerbong():
	...
	var jarak_offscreen = SPAWN_GERBONG_OFFSITE
	
	# 4. Posisikan gerbong baru tepat di belakang gerbong terakhir
	gerbong_fisika.global_position.x = node_paling_belakang.global_position.x - jarak_offscreen
